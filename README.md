...
K8S Capacity Planner is a dark-site Kubernetes capacity dashboard. It runs outside the target cluster, lets an authenticated administrator add mounted kubeconfig files, stores reports in local SQLite, and bundles Kubernetes v1.36 guidance into the application image.
...
- Capacity flow of total Node Capacity, Node Allocatable, scheduled requests, and raw remaining capacity.
- Concurrent rolling-update capacity from Deployment template requests and configured `maxSurge`, using one conservative rollout envelope per namespace.
...
For a Docker-only dark site, download the architecture-specific Docker archive from the `v0.1.13` release, verify its checksum, and load it directly:

```sh
# x86_64 host
sha256sum -c kcp-0.1.13-linux-amd64.docker.tar.sha256
docker load --input kcp-0.1.13-linux-amd64.docker.tar

# ARM64 host
sha256sum -c kcp-0.1.13-linux-arm64.docker.tar.sha256
docker load --input kcp-0.1.13-linux-arm64.docker.tar
```

Prepare these files on the dashboard host:

- TLS certificate and key for the dashboard endpoint.
- Bootstrap administrator password file with a non-empty value.
...
The image includes `kubectl v1.36.0` for a read-only connection check using the same mounted kubeconfig path available to KCP:

```sh
docker run --rm \
  --volume /srv/kcp/clusters:/run/kcp/clusters:ro \
  --entrypoint kubectl kcp:0.1.13 \
  --kubeconfig /run/kcp/clusters/production.kubeconfig \
  version --request-timeout=10s
```

Add `--context <context-name>` when the kubeconfig does not use the intended context as its current context. This command only reads the Kubernetes version endpoint; it does not modify the cluster.

For basic network reachability diagnostics, the image also includes `ping`:

```sh
docker run --rm --entrypoint ping kcp:0.1.13 -c 3 <kubernetes-api-host-or-ip>
```
