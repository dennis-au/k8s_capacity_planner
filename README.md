...
For a Docker-only dark site, download the architecture-specific Docker archive from the `v0.1.17` release, verify its checksum, and load it directly:

```sh
# x86_64 host
sha256sum -c kcp-0.1.17-linux-amd64.docker.tar.sha256
docker load --input kcp-0.1.17-linux-amd64.docker.tar

# ARM64 host
sha256sum -c kcp-0.1.17-linux-arm64.docker.tar.sha256
docker load --input kcp-0.1.17-linux-arm64.docker.tar
```
...
```sh
docker run --rm --entrypoint ping kcp:0.1.17 -c 3 <kubernetes-api-host-or-ip>
```
