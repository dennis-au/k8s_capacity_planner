...
def create_app(
    ...
    store.migrate()
    bootstrap = inspect_kubeconfig(config.kubeconfig_file, config.kube_context, config.kube_api_ip)
    store.bootstrap_cluster(
        name="Configured cluster",
        kubeconfig_file=str(config.kubeconfig_file),
        kube_context=bootstrap.context,
        endpoint=bootstrap.endpoint,
        api_ip=config.kube_api_ip,
    )
...
_OVERVIEW_TEMPLATE = """{% if not record %}<section class="empty"><div class="empty-copy"><p class="eyebrow">First report</p><h2>No snapshots collected</h2><p>Confirm the active cluster connection, then collect the first read-only snapshot.</p><a class="button-link" href="{{ url_for('clusters') }}">Review clusters</a></div><dl class="empty-details"><div><dt>Active cluster</dt><dd>{{ connection.name if connection else 'Not configured' }}</dd></div><div><dt>API endpoint</dt><dd>{{ connection.endpoint if connection else 'Unavailable' }}</dd></div><div><dt>Collection mode</dt><dd>Read-only</dd></div></dl></section>{% else %}
