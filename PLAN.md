...
## Milestone 1: Cluster Access Readiness

- Confirm `lab110` is reachable at `192.168.0.110:6443`.
- Confirm the `kcp-reader` service account can read the APIs required by KCP: `/version`, nodes, namespaces, pods, quotas, LimitRanges, events, ReplicaSets, HPAs, and Metrics API resources when available.
- Generate a dedicated kubeconfig with the API IP and CA data required for TLS verification.
- Record any unavailable API as a known partial-data condition.
